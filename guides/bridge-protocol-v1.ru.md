...
- Existing domain enum fields, которые уже отражают C++/broker naming, могут
  сохранить текущее `UPPER_SNAKE_CASE` spelling в этом draft, например `BUY`,
  `SELL`, `SPRINT`, `INTRADE_BAR` и `DEMO`.
- Responses и events должны использовать canonical spelling из протокола.
- Requests могут принимать documented aliases для compatibility, включая
  case-insensitive aliases, но bridges должны нормализовать их перед хранением
  или emit.
...
- Public JSON field names используют `lower_snake_case`; protocol-native enum
  values используют `lower_snake_case`; existing DTO/platform enum fields могут
  сохранять `UPPER_SNAKE_CASE` canonical spelling во время draft.
...
- Большие MT4/MT5 raw buffers могут использовать advertised compact/binary
  encoding.
...
- Точная durable subscription storage model для event replay между restarts
  bridge.
- Должен ли stable v1 нормализовать existing `UPPER_SNAKE_CASE` DTO enum fields
  в `lower_snake_case` или сохранить текущее DTO-compatible spelling.
- Точный binary payload format, когда compact MT4/MT5 buffers потребуют
  production interoperability, а не draft shape.
