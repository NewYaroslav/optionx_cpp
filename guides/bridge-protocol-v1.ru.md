...
- Stable v1 canonical output должен также нормализовать DTO/platform-style enum
  values в `lower_snake_case`, например `buy`, `sell`, `sprint`,
  `intrade_bar` и `demo`.
- Во время draft и migration period existing domain enum fields все еще могут
  встречаться в текущем `UPPER_SNAKE_CASE` spelling в examples или older
  bridges, например `BUY`, `SELL`, `SPRINT`, `INTRADE_BAR` и `DEMO`.
...
- Public JSON field names используют `lower_snake_case`; protocol-native enum
  values используют `lower_snake_case`; stable v1 должен нормализовать existing
  DTO/platform enum values в `lower_snake_case` на output, принимая и
  `UPPER_SNAKE_CASE`, и `lower_snake_case` на input.
