...
Внешние идентификаторы являются opaque strings в wire protocol, даже если
текущий C++ DTO хранит их как целые числа. Это сохраняет контракт устойчивым
для JavaScript clients, broker IDs, UUIDs, composite IDs и будущих более широких
integer types. Реализации могут парсить локально сгенерированные numeric
strings обратно в integer DTO fields на границе adapter.
...
MVP должен реализовать только `default`, `account_id` и опционально `all`.
