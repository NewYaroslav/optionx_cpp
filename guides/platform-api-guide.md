...
- Built-in default key предназначен для разработки/tests. Production code
  ...
  `has_custom_key()`.
- IV and generated AES keys use the operating-system random source; the
  default key is still a shared fallback and does not provide production-grade
  secret isolation by itself.
