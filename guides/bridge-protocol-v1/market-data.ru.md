...
subscriptions, и он получает события только по ним. Live subscriber обычно
является authenticated WebSocket connection, named-pipe client или application
component. Обычные HTTP clients обычно опрашивают history/query commands и не
получают live events. `subscriber_id` optional; если он не задан, bridge
...
`client_subscription_key` scoped к authenticated client и помогает сделать
...
Market-data prices, volumes и spreads используют shared decimal-string wire rule.
Они не являются `MoneyValue` objects, если concrete provider schema не говорит
иначе. Times используют integer milliseconds.
...
Open bar updates могут иметь один и тот же `open_time_ms` много раз до закрытия
бара. Providers должны добавлять `source_seq`, `bar_revision` или оба поля,
когда источник может дать стабильный ordering для bar revisions.
