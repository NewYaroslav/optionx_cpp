...
- Unknown client notification methods должны игнорироваться или отклоняться
  ...
  notifications only.
- `protocol.hello` является unversioned bootstrap method.
...
- Named-pipe и другие session transports выбирают protocol version во время
  initial handshake и затем сохраняют его на всю session.
