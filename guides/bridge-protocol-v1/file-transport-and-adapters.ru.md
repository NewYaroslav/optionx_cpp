...
Префикс `mt2trade_all`, судя по наблюдениям, отправляет signal на всех
настроенных brokers. Второе поле это opaque encoded payload. Несколько
captured samples отличаются при изменении martingale settings, но payload не
надо считать decoded public contract, пока encoding не понят и не покрыт
тестами.
...
- Можно ли безопасно декодировать MT2Trading `mt2trade_all` payloads настолько,
