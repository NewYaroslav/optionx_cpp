...
- `TradeRecordDB` intentionally stores a 32-bit linear `trade_id` in the low
  32 bits of its composite key. Do not convert the DB key to 64-bit without a
  separate storage redesign. A future PR may still review why
  `TradeRequest::trade_id`, `TradeResult::trade_id`, `TradeResultQuery::trade_id`
  and `TradeRecord::trade_id` expose `uint64_t`, and whether the wider DTO type
  is intentional API propagation or historical drift.
- `TradeRecord::assign_result()` is not a true partial-result merge. It
  preserves request/account identity when the corresponding `TradeResult`
  fields are unspecified, but other result-state fields are copied as-is. If a
  future feature needs `merge_partial_result()`, add an explicit patch DTO or
  presence flags first; do not add tests that promise preservation of every
  known result field while `TradeResult` cannot distinguish "zero value" from
  "field absent".
