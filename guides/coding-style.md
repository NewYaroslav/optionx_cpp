...
- `optionx::modules` - base modules/managers.
...
| Base classes | `Base<Name>` | `BaseModule`, `BaseBridge` |
| Managers/modules | PascalCase + `Manager`/`Module` | `AuthManager`, `HttpClientModule` |
...
Не бросай исключения из module `process()`/callbacks без явной обработки:
...
`BaseHttpClientModule.hpp`, `ServiceSessionDB.hpp`.
...
- `std::shared_ptr<T>` - shared domain state/events, например account info,
  `TradeRequestEvent`.
- Raw pointers - non-owning registered modules/listeners; lifetime должен быть
...
- Не отдавай mutable reference на internal state без существующего проекта
  pattern.
- Не регистрируй raw module pointer, если объект может умереть раньше platform.
