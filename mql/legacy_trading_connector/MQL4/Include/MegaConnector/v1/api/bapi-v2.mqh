...
private:
    ...
    bool send_mt4_connected() {
        ...
        json_body += IntegerToString(config.api_version);
        if (IsConnected()) json_body += ",\"d\":{\"status\":1}}";
        else json_body += ",\"d\":{\"status\":0}}";
        return write(json_body);
    }
    
    void update_mt4_connected() {
        if (is_last_mt4_connected != IsConnected()) {
            is_last_mt4_connected = IsConnected();
            send_mt4_connected();
        }
    }
...
public:
     ...
     */
    virtual void on_error(const int error_code);
    ...
    bool connect(string api_pipe_name) {
        if (is_connected) return true;
        config.pipe_name = api_pipe_name;
        return connect();
    }
    ...
    */
    void update(const int delay) {
        ...
        // отправляем пинг раз в 'config.ping_tick' тиков
        update_ping(delay);
                        ...
                        if (data_array != NULL) {
                            ...
                            for (int i = 0; i < accounts; ++i) {
                                ...
                                */
                                ...
                                account_info.is_connect = (connect_status == 1);
     ...
     * \param delay Задержка между вызовами функции
     */
    void update_v2(const int delay) {
        if (connect()) {
            update(delay);
        }
    }
    ...
    */
    void close() {
        if (is_connected) {
            ...
            for (int i = 0; i < len; ++i) {
                account_list[i].is_connect = false;
            }
            on_disconnect();
        }
        ping_tick = 0;
