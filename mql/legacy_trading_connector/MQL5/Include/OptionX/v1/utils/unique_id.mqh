...
//+------------------------------------------------------------------+
/*
string get_str_signal_id(const string _app_id, string &_signal_id) {
    _signal_id = get_str_unique_id();
    return _app_id + ":" + _signal_id;
}
*/
...
//+------------------------------------------------------------------+
/*
bool check_app_id(const string _app_id, const string _unique_id, string &_signal_id) {
    string result[];               // массив для получения строк
    const ushort u_sep = StringGetCharacter(":", 0);
    const int k = StringSplit(_unique_id, u_sep, result);
    if (k != 2 || result[0] != _app_id) {
        ArrayFree(result);
        return false;
    }
    _signal_id = result[1];
